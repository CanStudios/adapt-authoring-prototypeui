  ...
  */
  preload(app, resolve, reject) {
    ...
    server.root.createChildRouter('prototype')._router
        ...
        .get('/auth/login', this.handleLogin);
  ...
  */
  handleUsers(req, res, next) {
      ...
      new Requester('/users').get()
          ...
          .then(() => {res.render(path.join(__dirname, '../views/users'), { users, error });});
  }
  /**
  ...
  handleLogin(req, res, next) {
    new Requester('/auth/login').get()
      .then(() => {res.render(path.join(__dirname, '../views/login'));});
  }
}
